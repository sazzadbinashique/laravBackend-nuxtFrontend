  ...
  <div class="container mx-auto h-min-100">
    <div class="w-full bg-green-100 p-6 rounded-md text-center">
      ...
      <p class="text-gray-700 mt-4">
        This is the EMI Calculator page where we describe our application.
      </p>
      <!-- Add a Button for Contact -->
      <div class="mt-6">
        <NuxtLink to="/emi" class="inline-block bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
          Go to EMI
        </NuxtLink>
      </div>
    </div>
    <form @submit.prevent="calculateEmi" class="bg-cyan-100 shadow-md rounded px-8 pt-6 pb-8 mb-6">
      <div class="mb-4">
...
export default {
  ...
  methods: {
    calculateEmi() {
      ...
      for (let month = 1; month <= gracePeriodMonths; month++) {
        ...
        results.push({
          ...
          payment_date: this.addMonthsNoOverflow(new Date(this.disbursementDate), month),
      ...
      for (let month = 1; month <= remainingTenure; month++) {
        ...
        results.push({
          ...
          disbursement_date: this.disbursementDate,
          payment_date: this.addMonthsNoOverflow(new Date(this.disbursementDate), month + gracePeriodMonths),
    ...
    addMonthsNoOverflow(date, months) {
      ...
      // Adjust for overflow in February
      if (newDate.getMonth() === 1 && date.getDate() > 28) {
        newDate.setDate(Math.min(newDate.getDate(), 28)); // Set to 28 if it overflows
      }
